...
dataSubgraficas = pd.read_csv('/Users/danielabugeda/Documents/ITAM/Tesis/Códigos Octubre/Versiones Finales/archivo_datos_sin_ciclos.txt')
resultados = []
...
for i, fila in dataSubgraficas.iterrows():
    ...
    # Guardar la confianza real y eliminarla de la WoT
    ...
    # Tidal Trust
    st_tt = time.perf_counter()
    confianza_tt = fn.tidalTrust(WoT, fuente, sumidero)
    et_tt = time.perf_counter()
    execution_time_tt = st_tt - et_tt

    # Edmonds Karp con delta constante
    st_edkp1 = time.perf_counter()
    delta1 = {0: 0, 1: .90, 2: .9, 3: .90, 4: .90, 5:.90, 6:.90, 7: .90, 8: .90, 9: .90, 10: .90}
...
resultados.to_csv('/Users/danielabugeda/Documents/ITAM/Tesis/Códigos Octubre/Versiones Finales/estimaciones.txt', header=True, index=False, sep=',')